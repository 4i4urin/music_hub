...
#ifndef _ESP_HTTP_SERVER_H_MY
#define _ESP_HTTP_SERVER_H_MY
 ...
 *             This call MUST have a corresponding call to esp_http_client_cleanup_user when the operation is complete.
 *
...
/**
 * @brief      Invoke this function after `esp_http_client_init_user` and all the options calls are made, and will perform the
 *             transfer as described in the options. It must be called with the same esp_http_client_handle_t as input as the esp_http_client_init_user call returned.
 *             esp_http_client_perform_user performs the entire request in either blocking or non-blocking manner. By default, the API performs request in a blocking manner and returns when done,
 ...
 *             the user may call this API multiple times unless request & response is complete or there is a failure. To enable non-blocking esp_http_client_perform_user(), `is_async` member of esp_http_client_config_t
 *             must be set while making a call to esp_http_client_init_user() API.
 *             You can do any amount of calls to esp_http_client_perform_user while using the same esp_http_client_handle_t. The underlying connection may be kept open if the server allows it.
 ...
 *             Just note that you will have to use `esp_http_client_set_**` between the invokes to set options for the following esp_http_client_perform_user.
 *
 ...
 *             This function include `esp_http_client_open_user` -> `esp_http_client_write_user` -> `esp_http_client_fetch_headers_user` -> `esp_http_client_read_user` (and option) `esp_http_client_close_user`.
 *
...
/**
 * @brief      Set post data, this function must be called before `esp_http_client_perform_user`.
...
/**
 * @brief      Set http request header, this function must be called after esp_http_client_init_user and before any
 *             perform function
 *
 * @param[in]  client  The esp_http_client handle
 ...
 *             This function must be called after `esp_http_client_init_user`.
 *
 * @param[in]  client  The esp_http_client handle
 * @param[in]  key     The header key
...
/**
 * @brief     This function will write data to the HTTP connection previously opened by esp_http_client_open_user()
 *
 * @param[in]  client  The esp_http_client handle
 * @param      buffer  The buffer
 * @param[in]  len     This value must not be larger than the write_len parameter provided to esp_http_client_open_user()
 *
 * @return
 *     - (-1) if any errors
...
/**
 * @brief      This function need to call after esp_http_client_open_user, it will read from http stream, process all receive headers
 *
 * @param[in]  client  The esp_http_client handle
 *
 * @return
 ...
 */
bool esp_http_client_is_chunked_response_user(esp_http_client_handle_t client);
...
/**
 * @brief      Get http response status code, the valid value if this function invoke after `esp_http_client_perform_user`
 *
 * @param[in]  client  The esp_http_client handle
 *
 * @return     Status code
 ...
 *             the valid value if this function invoke after `esp_http_client_perform_user`
 *
 * @param[in]  client  The esp_http_client handle
 *
 * @return
 ...
 *             It is the opposite of the esp_http_client_init_user function and must be called with the same handle as input that a esp_http_client_init_user call returned.
 ...
 *             This is a helper API which internally calls `esp_http_client_read_user` multiple times till the end of data is reached or till the buffer gets full.
 *
 * @param[in]  client   The esp_http_client handle
 * @param      buffer   The buffer
 ...
 *              As no additional user-supplied buffer is required, this may be preferrable to `esp_http_client_read_user_response_user` in situations where the content of the response may be ignored.
 *
 * @param[in]  client  The esp_http_client handle
