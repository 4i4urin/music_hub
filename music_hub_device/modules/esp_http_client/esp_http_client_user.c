...
static esp_err_t _clear_connection_info_user(esp_http_client_handle_t client);
/**
 * Default settings
 */
...
static esp_err_t esp_http_client_request_send_user(esp_http_client_handle_t client, int write_len);
static esp_err_t esp_http_client_connect_user(esp_http_client_handle_t client);
static esp_err_t esp_http_client_send_post_data_user(esp_http_client_handle_t client);

static esp_err_t http_dispatch_event_user(esp_http_client_t *client, esp_http_client_event_id_t event_id, void *data, int len)
{
...
static int http_on_message_begin_user(http_parser *parser)
{
...
static int http_on_url_user(http_parser *parser, const char *at, size_t length)
{
    ESP_LOGD(TAG, "http_on_url");
    return 0;
}

static int http_on_status_user(http_parser *parser, const char *at, size_t length)
{
    return 0;
}

static int http_on_header_event_user(esp_http_client_handle_t client)
{
    if (client->current_header_key != NULL && client->current_header_value != NULL) {
        ...
        http_dispatch_event_user(client, HTTP_EVENT_ON_HEADER, NULL, 0);
...
static int http_on_header_field_user(http_parser *parser, const char *at, size_t length)
{
    esp_http_client_t *client = parser->data;
    http_on_header_event_user(client);
    http_utils_append_string(&client->current_header_key, at, length);

    return 0;
}

static int http_on_header_value_user(http_parser *parser, const char *at, size_t length)
{
...
static int http_on_headers_complete_user(http_parser *parser)
{
    esp_http_client_handle_t client = parser->data;
    http_on_header_event_user(client);
...
static int http_on_body_user(http_parser *parser, const char *at, size_t length)
{
    esp_http_client_t *client = parser->data;
    ...
    if (client->response->buffer->output_ptr) {
        ...
        /* Do not cache body when http_on_body is called from esp_http_client_perform_user */
    ...
    http_dispatch_event_user(client, HTTP_EVENT_ON_DATA, (void *)at, length);
    return 0;
}

static int http_on_message_complete_user(http_parser *parser)
{
...
static int http_on_chunk_complete_user(http_parser *parser)
{
    ESP_LOGD(TAG, "http_on_chunk_complete_user");
    return 0;
}

static int http_on_chunk_header_user(http_parser *parser)
{
    esp_http_client_handle_t client = parser->data;
    client->response->chunk_length = parser->content_length;
    ESP_LOGD(TAG, "http_on_chunk_header_user, chunk_length");
    return 0;
}

esp_err_t esp_http_client_set_header_user(esp_http_client_handle_t client, const char *key, const char *value)
{
    return http_header_set(client->request->headers, key, value);
}

esp_err_t esp_http_client_get_header_user(esp_http_client_handle_t client, const char *key, char **value)
{
    return http_header_get(client->request->headers, key, value);
}

esp_err_t esp_http_client_delete_header_user(esp_http_client_handle_t client, const char *key)
{
...
static esp_err_t _set_config_user(esp_http_client_handle_t client, const esp_http_client_config_t *config)
{
...
error:
    _clear_connection_info_user(client);
    return ret;
}
...
static esp_err_t _clear_auth_data_user(esp_http_client_handle_t client)
{
...
static esp_err_t esp_http_client_prepare_user(esp_http_client_handle_t client)
{
    ...
    if (client->connection_info.username) {
        ...
        if (auth_response) {
            ...
            esp_http_client_set_header_user(client, "Authorization", auth_response);
            free(auth_response);
        }
    }
    return ESP_OK;
...
static char *_get_host_header_user(char *host, int port)
{
    int err = 0;
    char *host_name;
...
esp_http_client_handle_t esp_http_client_init_user(const esp_http_client_config_t *config)
{
    ...
    if (_set_config_user(client, config) != ESP_OK) {
    ...
    if (config->host != NULL && config->path != NULL) {
        ...
        host_name = _get_host_header_user(client->connection_info.host, client->connection_info.port);
        if (host_name == NULL) {
            ESP_LOGE(TAG, "Failed to allocate memory for host header");
            goto error;
        }
        _success = (
            (esp_http_client_set_header_user(client, "User-Agent", user_agent) == ESP_OK) &&
            (esp_http_client_set_header_user(client, "Host", host_name) == ESP_OK)
        );
        free(host_name);
        if (!_success) {
        ...
        if (esp_http_client_set_url_user(client, config->url) != ESP_OK) {
     ...
     * used in esp_http_client_read_user API only. For esp_http_perform we shall disable
     * this as data will be processed by event handler */
    client->cache_data_in_fetch_hdr = 1;

    client->parser_settings->on_message_begin = http_on_message_begin_user;
    client->parser_settings->on_url = http_on_url_user;
    client->parser_settings->on_status = http_on_status_user;
    client->parser_settings->on_header_field = http_on_header_field_user;
    client->parser_settings->on_header_value = http_on_header_value_user;
    client->parser_settings->on_headers_complete = http_on_headers_complete_user;
    client->parser_settings->on_body = http_on_body_user;
    client->parser_settings->on_message_complete = http_on_message_complete_user;
    client->parser_settings->on_chunk_complete = http_on_chunk_complete_user;
    client->parser_settings->on_chunk_header = http_on_chunk_header_user;
...
error:
    esp_http_client_cleanup_user(client);
    return NULL;
}

static void esp_http_client_cached_buf_cleanup_user(esp_http_buffer_t *res_buffer)
{
...
esp_err_t esp_http_client_cleanup_user(esp_http_client_handle_t client)
{
    if (client == NULL) {
        return ESP_FAIL;
    }
    ...
    if (client->response) {
        ...
        if (client->response->buffer) {
            ...
            esp_http_client_cached_buf_cleanup_user(client->response->buffer);
        }
    ...
    switch (client->response->status_code) {
        ...
        case HttpStatus_PermanentRedirect:
            if (client->disable_auto_redirect) {
                http_dispatch_event_user(client, HTTP_EVENT_REDIRECT, NULL, 0);
            } else {
                if (esp_http_client_set_redirection_user(client) != ESP_OK){
                    return ESP_FAIL;
                };
            }
            break;
        case HttpStatus_Unauthorized:
            esp_http_client_add_auth_user(client);
    }
    return ESP_OK;
}

esp_err_t esp_http_client_set_url_user(esp_http_client_handle_t client, const char *url)
{
    esp_err_t ret = ESP_OK;
    char *old_host = NULL;
    ...
    if (old_host && client->connection_info.host
        ...
        if (esp_http_client_set_header_user(client, "Host", client->connection_info.host) != ESP_OK) {
            free(old_host);
...
int esp_http_client_get_errno_user(esp_http_client_handle_t client)
{
    if (!client) {
...
esp_err_t esp_http_client_set_method_user(esp_http_client_handle_t client, esp_http_client_method_t method)
{
    client->connection_info.method = method;
    return ESP_OK;
}

esp_err_t esp_http_client_set_timeout_ms_user(esp_http_client_handle_t client, int timeout_ms)
{
    if (client == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
...
static int esp_http_client_get_data_user(esp_http_client_handle_t client)
{
...
bool esp_http_client_is_complete_data_received_user(esp_http_client_handle_t client)
{
...
int esp_http_client_read_user(esp_http_client_handle_t client, char *buffer, int len)
{
    esp_http_buffer_t *res_buffer = client->response->buffer;
    ...
    if (res_buffer->raw_len) {
        ...
        if (res_buffer->raw_len == 0) {
            esp_http_client_cached_buf_cleanup_user(res_buffer);
        }
    }
    ...
    while (need_read > 0 && is_data_remain) {
        ...
        if (rlen <= 0) {
            ...
            if (rlen < 0 && ridx == 0 && !esp_http_client_is_complete_data_received_user(client)) {
                http_dispatch_event_user(client, HTTP_EVENT_ERROR, esp_transport_get_error_handle(client->transport), 0);
                return ESP_FAIL;
            }
            return ridx;
        }
...
esp_err_t esp_http_client_perform_user(esp_http_client_handle_t client)
{
    esp_err_t err;
    do {
        if (client->process_again) {
            esp_http_client_prepare_user(client);
        }
        switch (client->state) {
        /* In case of blocking esp_http_client_perform_user(), the following states will fall through one after the after;
           in case of non-blocking esp_http_client_perform_user(), if there is an error condition, like EINPROGRESS or EAGAIN,
           then the esp_http_client_perform_user() API will return ESP_ERR_HTTP_EAGAIN error. The user may call
           esp_http_client_perform_user API again, and for this reason, we maintain the states */
            case HTTP_STATE_INIT:
                if ((err = esp_http_client_connect_user(client)) != ESP_OK) {
            ...
            case HTTP_STATE_CONNECTED:
                if ((err = esp_http_client_request_send_user(client, client->post_len)) != ESP_OK) {
            ...
            case HTTP_STATE_REQ_COMPLETE_HEADER:
                if ((err = esp_http_client_send_post_data_user(client)) != ESP_OK) {
                    if (client->is_async && errno == EAGAIN) {
                        return ESP_ERR_HTTP_EAGAIN;
                    }
                    http_dispatch_event_user(client, HTTP_EVENT_ERROR, esp_transport_get_error_handle(client->transport), 0);
            ...
            case HTTP_STATE_REQ_COMPLETE_DATA:
                ...
                int64_t ret = esp_http_client_fetch_headers_user(client);
                if (ret < 0) {
            ...
            case HTTP_STATE_RES_ON_DATA_START:
                 ...
                 * request could be performed using native APIs */
                client->cache_data_in_fetch_hdr = 1;
                if ((err = esp_http_check_response_user(client)) != ESP_OK) {
                    ESP_LOGE(TAG, "Error response");
                    http_dispatch_event_user(client, HTTP_EVENT_ERROR, esp_transport_get_error_handle(client->transport), 0);
                    return err;
                }
                while (client->response->is_chunked && !client->is_chunk_complete) {
                    if (esp_http_client_get_data_user(client) <= 0) {
                        if (client->is_async && errno == EAGAIN) {
                            return ESP_ERR_HTTP_EAGAIN;
                        }
                ...
                http_dispatch_event_user(client, HTTP_EVENT_ON_FINISH, NULL, 0);
...
int64_t esp_http_client_fetch_headers_user(esp_http_client_handle_t client)
{
...
static esp_err_t esp_http_client_connect_user(esp_http_client_handle_t client)
{
    ...
    if ((err = esp_http_client_prepare_user(client)) != ESP_OK) {
    ...
    if (client->state < HTTP_STATE_CONNECTED) {
        ...
        client->state = HTTP_STATE_CONNECTED;
        http_dispatch_event_user(client, HTTP_EVENT_ON_CONNECTED, NULL, 0);
    }
    return ESP_OK;
}

static int http_client_prepare_first_line_user(esp_http_client_handle_t client, int write_len)
{
    if (write_len >= 0) {
        http_header_set_format(client->request->headers, "Content-Length", "%d", write_len);
    } else {
        esp_http_client_set_header_user(client, "Transfer-Encoding", "chunked");
    }
...
static esp_err_t esp_http_client_request_send_user(esp_http_client_handle_t client, int write_len)
{
    ...
    if (!client->first_line_prepared) {
        if ((first_line_len = http_client_prepare_first_line_user(client, write_len)) < 0) {
            return first_line_len;
        }
    ...
    if (client->data_write_left > 0) {
        /* sending leftover data from previous call to esp_http_client_request_send_user() API */
        int wret = 0;
        if (((wret = esp_http_client_write_user(client, client->request->buffer->data + client->data_written_index, client->data_write_left)) < 0)) {
    ...
    http_dispatch_event_user(client, HTTP_EVENT_HEADERS_SENT, NULL, 0);
...
static esp_err_t esp_http_client_send_post_data_user(esp_http_client_handle_t client)
{
    ...
    int wret = esp_http_client_write_user(client, client->post_data + client->data_written_index, client->data_write_left);
    if (wret < 0) {
        return wret;
    }
    client->data_write_left -= wret;
...
esp_err_t esp_http_client_open_user(esp_http_client_handle_t client, int write_len)
{
    ...
    if ((err = esp_http_client_request_send_user(client, write_len)) != ESP_OK) {
        http_dispatch_event_user(client, HTTP_EVENT_ERROR, esp_transport_get_error_handle(client->transport), 0);
        return err;
    }
    return ESP_OK;
...
int esp_http_client_write_user(esp_http_client_handle_t client, const char *buffer, int len)
{
    if (client->state < HTTP_STATE_REQ_COMPLETE_HEADER) {
        return ESP_FAIL;
    }
...
esp_err_t esp_http_client_close_user(esp_http_client_handle_t client)
{
    if (client->state >= HTTP_STATE_INIT) {
        http_dispatch_event_user(client, HTTP_EVENT_DISCONNECTED, esp_transport_get_error_handle(client->transport), 0);
        client->state = HTTP_STATE_INIT;
...
esp_err_t esp_http_client_set_post_field_user(esp_http_client_handle_t client, const char *data, int len)
{
    ...
    if (client->post_data) {
        char *value = NULL;
        if ((err = esp_http_client_get_header_user(client, "Content-Type", &value)) != ESP_OK) {
            return err;
        }
        if (value == NULL) {
            err = esp_http_client_set_header_user(client, "Content-Type", "application/x-www-form-urlencoded");
        }
    } else {
        client->post_len = 0;
        err = esp_http_client_set_header_user(client, "Content-Type", NULL);
    }
    return err;
}

int esp_http_client_get_post_field_user(esp_http_client_handle_t client, char **data)
{
    if (client->post_data) {
...
int esp_http_client_get_status_code_user(esp_http_client_handle_t client)
{
    return client->response->status_code;
}

int64_t esp_http_client_get_content_length_user(esp_http_client_handle_t client)
{
    return client->response->content_length;
}
...
esp_http_client_transport_t esp_http_client_get_transport_type_user(esp_http_client_handle_t client)
{
...
int esp_http_client_read_user_response_user(esp_http_client_handle_t client, char *buffer, int len)
{
    int read_len = 0;
    while (read_len < len) {
        int data_read = esp_http_client_read_user(client, buffer + read_len, len - read_len);
        if (data_read <= 0) {
            return read_len;
        }
        read_len += data_read;
...
esp_err_t esp_http_client_flush_response_user(esp_http_client_handle_t client, int *len)
{
    if (client == NULL) {
        ESP_LOGE(TAG, "client must not be NULL");
        return ESP_ERR_INVALID_ARG;
    ...
    int read_len = 0;
    while (!esp_http_client_is_complete_data_received_user(client)) {
        int data_read = esp_http_client_get_data_user(client);
        if (data_read < 0) {
            return ESP_FAIL;
        }
        read_len += data_read;
...
esp_err_t esp_http_client_get_url_user(esp_http_client_handle_t client, char *url, const int len)
{
    if (client == NULL || url == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t esp_http_client_get_chunk_length_user(esp_http_client_handle_t client, int *len)
{
    if (client == NULL || len == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    if (esp_http_client_is_chunked_response_user(client)) {
...
int esp_http_client_poll_read_user(esp_http_client_handle_t client)
{
    if (client == NULL) {
        return ESP_FAIL;
    }
    return esp_transport_poll_read(client->transport, client->timeout_ms);
}


int esp_http_client_read(esp_http_client_handle_t client, char * buffer_r, int max_buf_len)
{
    if (client->state < HTTP_STATE_REQ_COMPLETE_HEADER) {
        return ESP_FAIL;
    }
    ...
    if (client->response->content_length > max_buf_len)
        memcpy(buffer_r, client->response->buffer->raw_data, max_buf_len);
    else
        memcpy(buffer_r, client->response->buffer->raw_data, client->response->content_length);
